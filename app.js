...
const elements = {
  ...
  dTransactionType:         document.getElementById('d-transaction-type'),
...
function setupUserEnvironment() {
  ...
  elements.userRoleBadge.innerText   = isAdmin()
    ? 'สิทธิ์: แอดมิน (แก้ไขข้อมูลได้)'
    : 'สิทธิ์: ผู้บริหาร (ดูอย่างเดียว)';
...
function applyRoleBasedUI() {
  ...
  // Transfers are not part of the current budget-based workflow.
  document.querySelectorAll('[data-tab="transfers"], #tab-transfers').forEach(el => {
...
function applyTheme(theme) {
  ...
  // Update icon only (top navbar uses icon-only round button)
  const iconEl = elements.themeToggleBtn ? elements.themeToggleBtn.querySelector('i') : null;
  if (iconEl) {
...
// --- TRANSACTION HELPERS ---
// =============================================================
function getTransactionType(record) {
  return record?.transactionType === 'in' ? 'in' : 'out';
}

function isMoneyIn(record) {
  return getTransactionType(record) === 'in';
}

function getSignedNetAmount(record) {
  const amount = parseFloat(record?.netAmount) || 0;
  return isMoneyIn(record) ? amount : -amount;
}

function getBalanceEffect(record) {
  if (!record || record.status === 'Cancelled') return 0;
  return getSignedNetAmount(record);
}

function getTransactionBadge(record) {
  return isMoneyIn(record)
    ? '<span class="badge badge-success">โอนเข้า</span>'
    : '<span class="badge badge-danger">โอนออก</span>';
}

function getShopBalanceSummary(shopId, excludeId = '') {
  const records = AppState.disbursements
    .filter(d => d.shopId === shopId && d.id !== excludeId && d.status !== 'Cancelled');
  const moneyIn = records
    .filter(isMoneyIn)
    .reduce((s, d) => s + (parseFloat(d.netAmount) || 0), 0);
  const moneyOut = records
    .filter(d => !isMoneyIn(d))
    .reduce((s, d) => s + (parseFloat(d.netAmount) || 0), 0);

  return { moneyIn, moneyOut, netEffect: moneyIn - moneyOut };
}
...
function switchTab(tabName) {
  if (tabName === 'transfers') tabName = 'dashboard';
  if (tabName === 'admin' && !isAdmin()) tabName = 'dashboard';
...
function renderDashboard() {
  ...
  const totalOut = d
    .filter(r => !isMoneyIn(r))
    .reduce((s, r) => s + (r.netAmount || 0), 0);
  const paidOutSum = paid
    .filter(r => !isMoneyIn(r))
    .reduce((s, r) => s + (r.netAmount || 0), 0);
  const totalBudget = AppState.shops.reduce((s, shop) => s + (shop.totalBudget || 0), 0);
  const netTransfer = d
    .filter(r => r.status !== 'Cancelled')
    .reduce((s, r) => s + getBalanceEffect(r), 0);
  const remainingBudget = totalBudget + netTransfer;
  const totalIn = d
    .filter(r => isMoneyIn(r) && r.status !== 'Cancelled')
    .reduce((s, r) => s + (r.netAmount || 0), 0);
  const totalOutActive = d
    .filter(r => !isMoneyIn(r) && r.status !== 'Cancelled')
    .reduce((s, r) => s + (r.netAmount || 0), 0);

  if (elements.statTotalDisbursed) elements.statTotalDisbursed.innerText = formatCurrency(totalOut);
  if (elements.statTotalPaid)      elements.statTotalPaid.innerText      = formatCurrency(paidOutSum);
  if (elements.statPaidCount)      elements.statPaidCount.innerText      = `${paid.filter(r => !isMoneyIn(r)).length} รายการ`;
  if (elements.statTotalPending)   elements.statTotalPending.innerText   = formatCurrency(remainingBudget);
  if (elements.statPendingCount)   elements.statPendingCount.innerText   = `งบรวม ${formatCurrency(totalBudget)} + โอนเข้า ${formatCurrency(totalIn)} - โอนออก ${formatCurrency(totalOutActive)}`;
...
function renderRecentTable() {
  ...
  if (recent.length === 0) {
    tbody.innerHTML = '<tr><td colspan="11" class="text-center text-muted py-4">ไม่มีข้อมูลการเบิกจ่าย</td></tr>';
    return;
  }
  tbody.innerHTML = recent.map(r => {
    ...
    return `<tr>
      ...
      <td class="text-center">${getTransactionBadge(r)}</td>
    </tr>`;
  }).join('');
}
...
function renderMonthlyChart() {
  ...
  const monthlyData = {};
  AppState.disbursements.filter(r => !isMoneyIn(r)).forEach(r => {
    if (!r.date) return;
...
function renderDisbursementsTable() {
  ...
  if (pageData.length === 0) {
    tbody.innerHTML = '<tr><td colspan="12" class="text-center text-muted py-4">ไม่พบรายการตามเงื่อนไขที่กำหนด</td></tr>';
  } else {
...
function openAddDisbursementModal() {
  if (!isAdmin()) {
    showToast('บัญชีผู้บริหารเป็นโหมดดูอย่างเดียว ไม่สามารถเพิ่มรายการเบิกจ่ายได้', 'warning');
    return;
  }
  ...
  if (elements.dTransactionType) elements.dTransactionType.value = 'out';
...
function openEditDisbursementModal(id) {
  if (!isAdmin()) {
    showToast('บัญชีผู้บริหารเป็นโหมดดูอย่างเดียว ไม่สามารถแก้ไขรายการเบิกจ่ายได้', 'warning');
    return;
  }
  ...
  if (elements.dTransactionType) elements.dTransactionType.value = getTransactionType(r);
...
function updateDisbursementCalculation() {
  ...
  const transactionType = elements.dTransactionType?.value || 'out';
  ...
  if (shopId && transactionType === 'out') {
    const shop = AppState.shops.find(s => s.id === shopId);
    const currentId = elements.dId.value;
    const balance = getShopBalanceSummary(shopId, currentId);
    const remaining = (shop?.totalBudget || 0) + balance.netEffect;
  ...
  } else if (elements.budgetWarningBanner) {
    elements.budgetWarningBanner.style.display = 'none';
  }
}
...
async function handleDisbursementFormSubmit(e) {
  e.preventDefault();
  if (!isAdmin()) {
    showToast('บัญชีผู้บริหารเป็นโหมดดูอย่างเดียว ไม่สามารถแก้ไขข้อมูลได้', 'warning');
    return;
  }
  const subtotal  = parseFloat(elements.dSubtotal.value) || 0;
  ...
  const record = {
    ...
    transactionType: elements.dTransactionType?.value || 'out',
...
async function handleDeleteDisbursement(id) {
  if (!isAdmin()) {
    showToast('บัญชีผู้บริหารเป็นโหมดดูอย่างเดียว ไม่สามารถลบข้อมูลได้', 'warning');
    return;
  }
...
function renderShopsTab() {
  ...
  container.innerHTML = AppState.shops.map(shop => {
    const balance = getShopBalanceSummary(shop.id);
    const totalUsed = balance.moneyOut;
    const budget    = shop.totalBudget || 0;
    const remaining = budget + balance.moneyIn - balance.moneyOut;
      ...
      <div class="shop-card-footer">
        <div class="budget-info">
          <div class="budget-row">
            ...
            <span>โอนออก: <strong class="text-danger">${formatCurrency(totalUsed)}</strong></span>
            <span>โอนเข้า: <strong class="text-success">${formatCurrency(balance.moneyIn)}</strong></span>
...
function openAddShopModal() {
  if (!isAdmin()) {
    showToast('บัญชีผู้บริหารเป็นโหมดดูอย่างเดียว ไม่สามารถเพิ่มร้านค้าได้', 'warning');
    return;
  }
...
function openEditShopModal(id) {
  if (!isAdmin()) {
    showToast('บัญชีผู้บริหารเป็นโหมดดูอย่างเดียว ไม่สามารถแก้ไขร้านค้าได้', 'warning');
    return;
  }
  const shop = AppState.shops.find(s => s.id === id);
  if (!shop) return;
...
async function handleDeleteShop(id) {
  if (!isAdmin()) {
    showToast('บัญชีผู้บริหารเป็นโหมดดูอย่างเดียว ไม่สามารถลบร้านค้าได้', 'warning');
    return;
  }
...
function openShopHistoryModal(shopId) {
  ...
  const balance = getShopBalanceSummary(shopId);
  const totalUsed  = balance.moneyOut;
  const budget     = shop.totalBudget || 0;
  const remaining  = budget + balance.moneyIn - balance.moneyOut;
  const pct        = budget > 0 ? Math.min(100, (totalUsed / budget) * 100) : 0;
  ...
  if (shopDisbursements.length === 0) {
    elements.historyTbody.innerHTML = '<tr><td colspan="7" class="text-center text-muted py-4">ไม่มีประวัติการเบิกจ่ายของร้านค้านี้</td></tr>';
  } else {
  ...
  try {
    ...
    tbody.innerHTML = active.map(u => `
      <tr>
        ...
        <td>${u.displayName || '-'}</td>
        <td>${u.role === 'admin' ? '<span class="badge badge-danger">Admin</span>' : '<span class="badge badge-info">ผู้บริหาร (View Only)</span>'}</td>
        <td class="text-center">
...
function openAddUserModal() {
  if (!isAdmin()) {
    showToast('เฉพาะแอดมินเท่านั้นที่จัดการผู้ใช้งานได้', 'warning');
    return;
  }
...
function handleExportCsv() {
  ...
  const headers = ['วันที่','เลขที่เอกสาร','ร้านค้า','รายละเอียด','ประเภท','ยอดก่อนภาษี','VAT','หัก ณ ที่จ่าย','ยอดสุทธิ','ช่องทางการจ่าย','สถานะ'];
  const rows = filtered.map(r => {
    const shop = AppState.shops.find(s => s.id === r.shopId);
    return [
      r.date, r.ref, shop ? shop.name : '',
      `"${r.details.replace(/"/g, '""')}"`,
      isMoneyIn(r) ? 'โอนเข้า' : 'โอนออก',
...
function setupEventListeners() {
  ...
  elements.dTransactionType?.addEventListener('change', updateDisbursementCalculation);
