...
app.post('/api/disbursements', requireAuth, async (req, res) => {
  const { date, ref, shopId, transactionType, details, subtotal, vatType, vatAmount, whtRate, whtAmount, netAmount, paymentMethod, status } = req.body;
...
app.put('/api/disbursements/:id', requireAuth, async (req, res) => {
  ...
  if (fields.transactionType !== undefined) fields.transactionType = fields.transactionType === 'in' ? 'in' : 'out';
